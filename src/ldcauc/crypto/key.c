...
l_km_err embed_rootkey(ldacs_roles role, const char *as_ua, const char *sgw_ua) {
    ...
    // char *db_name = get_db_name(role, TODO);
    char *db_name = NULL;
    switch (role) {
        case LD_AS:
            db_name = get_db_name(LD_AS, as_ua);
            break;
        // case LD_GS:
        //     db_name = get_db_name(LD_GS, );
        //     break;
        case LD_SGW:
            db_name = get_db_name(LD_SGW, sgw_ua);
            break;
        default:
            return LD_ERR_KM_IMPORT_KEY;
    }
    if (!db_name) return LD_ERR_KM_IMPORT_KEY;
