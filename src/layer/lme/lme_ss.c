...
l_err send_auc_resp(void *args) {
    ...
    if (generate_auc_kdf(as_man->shared_random, &as_man->key_as_sgw_s_h, &as_man->key_as_gs_h, &as_man->key_as_gs_b)) {
        //进入错误状态
        return LD_ERR_INTERNAL;
    }
...
l_err handle_recv_msg(buffer_t *buf, const lme_as_man_t *as_man) {
    ...
    for (int i = 0; i < handler_size; i++) {
        if (handler[i].type == *buf->ptr) {
            return handler[i].callback(buf, (lme_as_man_t *) as_man);
        }
    }
